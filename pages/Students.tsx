      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={onBack} className="w-11 h-11 bg-white/20 backdrop-blur-md rounded-2xl text-white active:scale-90 transition-all border border-white/20 shadow-xl flex items-center justify-center">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                <p className="text-[10px] font-black text-white/80 uppercase tracking-widest leading-none">
          ...
          <div className="flex items-center gap-2">
            {isSelectionMode && (
              <button onClick={toggleSelectAll}
                className={`shrink-0 h-11 px-4 rounded-2xl transition-all active:scale-95 border flex items-center justify-center gap-2 font-black text-[11px] uppercase tracking-wider ${allFilteredSelected ? 'bg-white text-[#8D30F4] border-white shadow-xl' : 'bg-white/20 text-white border-white/20'}`}>
            ...
            <button onClick={() => { setIsSelectionMode(!isSelectionMode); if (isSelectionMode) setSelectedIds(new Set()); }}
              className={`shrink-0 w-11 h-11 rounded-2xl transition-all active:scale-95 border flex items-center justify-center ${isSelectionMode ? 'bg-white text-[#8D30F4] border-white shadow-xl' : 'bg-white/20 text-white border-white/20'}`}>
              {isSelectionMode ? <X size={20} strokeWidth={3} /> : <CheckCircle2 size={20} strokeWidth={2.5} />}
            </button>
            {!isSelectionMode && (
              <button onClick={onAddClick} className="premium-btn text-white px-5 py-3 rounded-2xl text-[12px] font-black flex items-center gap-2 shadow-xl active:scale-95 transition-all border border-white/20">
                <Plus size={16} strokeWidth={4} /> {t('add_student', lang)}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="relative group">
          ...
          <input type="text" placeholder={t('search_placeholder', lang)}
            className="w-full pl-12 pr-6 py-4 bg-white/95 backdrop-blur-md border-2 border-transparent rounded-[1.5rem] outline-none text-[#2D3142] placeholder:text-[#9B6DFF] font-black text-sm focus:border-[#8D30F4] shadow-xl transition-all"
      ...
      <div className="grid grid-cols-1 gap-2.5">
        {filteredStudents.map(student => (
          <div key={student.id} onClick={() => isSelectionMode ? toggleSelection(student.id) : onStudentClick(student)}
            className={`p-3 rounded-[1.5rem] border backdrop-blur-md transition-all flex items-center justify-between shadow-lg relative overflow-hidden ${isSelectionMode && selectedIds.has(student.id) ? 'bg-white text-[#8D30F4] border-[#8D30F4] scale-[1.01]' : 'bg-white/95 border-white/40 active:scale-[0.98]'}`}>
            <div className="flex items-center gap-4 flex-1 min-w-0">
              {isSelectionMode ? (
                ...
                <div className="w-12 h-12 rounded-2xl flex flex-col items-center justify-center border shrink-0 bg-[#F2EBFF] border-[#8D30F4]/10 text-[#8D30F4] shadow-inner">
                  <span className="text-[8px] font-black opacity-40 uppercase leading-none">Roll</span>
        ...
        {filteredStudents.length === 0 && !loading && (
          <div className="py-20 text-center bg-white/10 rounded-[3rem] border-2 border-dashed border-white/30 backdrop-blur-sm">
            <p className="text-white font-black text-[12px] uppercase tracking-widest">{t('no_students', lang)}</p>
          </div>
        )}
      </div>
      ...
      {isSelectionMode && selectedIds.size > 0 && (
        <div className="fixed bottom-[calc(env(safe-area-inset-bottom,0px)+95px)] left-1/2 -translate-x-1/2 w-[94%] max-w-md z-[150] animate-in slide-in-from-bottom-10">
          <div className="bg-white/95 backdrop-blur-xl rounded-[2.5rem] p-5 shadow-[0_30px_70px_rgba(46,11,94,0.5)] border border-[#8D30F4]/20 flex flex-col gap-4">
            <button onClick={() => setShowTemplateMenu(!showTemplateMenu)} className={`w-full flex items-center justify-between p-4.5 rounded-[1.5rem] text-sm font-black transition-all border-2 ${selectedTemplate ? 'bg-[#8D30F4]/5 border-[#8D30F4] text-[#8D30F4]' : 'bg-slate-50 border-slate-100 text-slate-400'}`}>
              ...
              <ChevronDown size={22} className={`transition-transform duration-300 ${showTemplateMenu ? 'rotate-180' : ''}`} />
            </button>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button onClick={handlePremiumSMS} disabled={sending || !selectedTemplate} className={`py-5 rounded-[1.5rem] flex items-center justify-center gap-2 font-black text-[12px] uppercase shadow-xl transition-all ${selectedTemplate ? 'premium-btn text-white' : 'bg-slate-100 text-slate-300 opacity-50'}`}>
                {sending ? <Loader2 className="animate-spin" size={18} /> : <MessageSquare size={18} fill="currentColor" />} 
                {lang === 'bn' ? 'সিস্টেম SMS' : 'System SMS'}
              </button>
              <button onClick={handleNativeSMS} disabled={!selectedTemplate} className={`py-5 rounded-[1.5rem] flex items-center justify-center gap-2 font-black text-[12px] uppercase shadow-xl transition-all ${selectedTemplate ? 'bg-[#2E0B5E] text-white' : 'bg-slate-100 text-slate-300 opacity-50'}`}>
                <Smartphone size={18} /> {t('native_sms', lang)}
              </button>
            </div>
            
            <div className="flex items-center justify-center gap-2 pt-1">
              <div className="w-2 h-2 rounded-full bg-[#8D30F4] animate-pulse"></div>
              <p className="text-[11px] font-black text-[#8D30F4] uppercase tracking-[0.2em]">{selectedIds.size} {t('selected', lang)}</p>
            </div>
          </div>
        </div>
      )}
