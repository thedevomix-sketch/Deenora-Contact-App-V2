    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="relative group">
        ...
        <input
          ...
          className="w-full pl-16 pr-6 py-4.5 bg-white/90 border-2 border-[#8D30F4]/20 rounded-[1.8rem] outline-none text-[#2D3142] placeholder:text-[#9B6DFF] font-black text-base focus:border-[#8D30F4] shadow-xl transition-all backdrop-blur-md"
          value={searchQuery}
      ...
      {searchQuery.length > 0 && (
        <div className="space-y-3 animate-in slide-in-from-top-4">
          <h2 className="text-[11px] font-black text-white uppercase tracking-[0.2em] px-4 drop-shadow-md">সার্চ ফলাফল</h2>
          {searchResults.map(student => (
            <div key={student.id} onClick={() => onStudentClick(student)} className="bg-white/95 p-4 rounded-3xl border-l-4 border-l-[#8D30F4] border border-white/20 flex items-center justify-between shadow-lg active:scale-[0.98] transition-transform">
              <div className="min-w-0 flex-1">
                <h3 className="font-black text-[#4B168A] text-[18px] font-noto truncate leading-tight">{student.student_name}</h3>
                <p className="text-[10px] text-[#A179FF] font-black uppercase mt-0.5 tracking-widest">{student.classes?.class_name || 'N/A'}</p>
              </div>
              <div className="w-10 h-10 bg-[#8D30F4] rounded-xl flex items-center justify-center text-white shadow-md">
      ...
      <div className="space-y-5">
        ...
        {loadingRecent && recentCalls.length === 0 ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => <div key={i} className="h-24 bg-white/30 animate-pulse rounded-3xl border border-white/10"></div>)}
          </div>
        ) : recentCalls.length > 0 ? (
          <div className="space-y-3">
            {recentCalls.map(call => (
              <div key={call.id} onClick={() => call.students && onStudentClick(call.students)} className="bg-white/95 p-4 rounded-3xl border border-white/40 flex items-center justify-between shadow-lg active:scale-[0.98] transition-all group backdrop-blur-lg">
                <div className="flex items-center gap-4 min-w-0 flex-1">
                  <div className="w-12 h-12 bg-[#F2EBFF] rounded-2xl flex items-center justify-center text-[#8D30F4] shrink-0 border border-[#8D30F4]/10 shadow-inner">
                ...
                <div className="flex items-center gap-2 shrink-0">
                   <div onClick={(e) => { e.stopPropagation(); call.students && initiateCall(call.students.guardian_phone) }} className="w-11 h-11 premium-btn text-white rounded-xl flex items-center justify-center shadow-md active:scale-90 transition-all">
