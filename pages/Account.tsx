...
import { LogOut, Camera, Loader2, Lock, User as UserIcon, ShieldCheck, Database, Phone, ChevronRight } from 'lucide-react';
...
const Account: React.FC<AccountProps> = ({ lang, setLang, onProfileUpdate, setView, isSuperAdmin, initialMadrasah }) => {
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true);
    try {
      await supabase.from('madrasahs').update({ name: newName.trim(), phone: newPhone.trim(), login_code: newLoginCode.trim() }).eq('id', madrasah.id);
      if (onProfileUpdate) onProfileUpdate();
      alert('Updated!');
    } catch (err) { alert('Failed'); } finally { setSaving(false); }
  };

  if (!madrasah) return <div className="py-24 text-center text-white font-black">Loading Account...</div>;

  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-24">
      <div className="flex items-center justify-between px-4">
        <h1 className="text-3xl font-black text-white font-noto drop-shadow-md">প্রোফাইল সেটিংস</h1>
        <div className="bg-white/20 backdrop-blur-md px-5 py-2.5 rounded-2xl border border-white/30 text-[12px] font-black text-white">SMS: {madrasah?.sms_balance || 0}</div>
      </div>
      ...
      <div className="bg-white/95 backdrop-blur-xl p-10 rounded-[3.5rem] border border-white/50 shadow-[0_40px_100px_rgba(46,11,94,0.4)] text-center space-y-10">
        <div className="flex flex-col items-center gap-6">
          <div className="relative">
            ...
            {!isSuperAdmin && (
              <button onClick={() => fileInputRef.current?.click()} className="absolute -bottom-2 -right-2 w-12 h-12 bg-[#8D30F4] text-white rounded-[1.2rem] shadow-2xl border-4 border-white flex items-center justify-center active:scale-90 transition-all"><Camera size={22} /></button>
            )}
            <input type="file" ref={fileInputRef} className="hidden" accept="image/*" />
          </div>
          <div>
            <h2 className="text-2xl font-black text-[#2E0B5E] font-noto leading-tight">{madrasah.name}</h2>
            <p className="text-[12px] font-black text-[#A179FF] uppercase tracking-[0.3em] mt-2">Madrasah Portal</p>
          </div>
        </div>
        ...
        <div className="space-y-6 text-left">
           ...
           <div className="space-y-4">
              <div className="bg-[#F2EBFF] p-6 rounded-[2rem] border-2 border-transparent focus-within:border-[#8D30F4]/30 transition-all">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-1 block mb-2">Admin Login Code</label>
                <div className="flex items-center gap-4"><Lock size={22} className="text-[#8D30F4]" /><input type="text" className="bg-transparent border-none outline-none font-black text-[#2E0B5E] text-lg tracking-[0.2em] w-full" value={newLoginCode} onChange={(e) => setNewLoginCode(e.target.value)} /></div>
              </div>
              <div className="bg-[#F2EBFF] p-6 rounded-[2rem] border-2 border-transparent focus-within:border-[#8D30F4]/30 transition-all">
                <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest px-1 block mb-2">Mobile Number</label>
                <div className="flex items-center gap-4"><Phone size={22} className="text-[#8D30F4]" /><input type="tel" className="bg-transparent border-none outline-none font-black text-[#2E0B5E] text-lg w-full" value={newPhone} onChange={(e) => setNewPhone(e.target.value)} /></div>
              </div>
           </div>
        </div>
        ...
        <button onClick={handleUpdate} disabled={saving} className="w-full py-6 premium-btn text-white font-black rounded-[2.2rem] shadow-[0_25px_60px_rgba(141,48,244,0.4)] active:scale-[0.98] transition-all flex items-center justify-center gap-4 text-xl">
           {saving ? <Loader2 className="animate-spin" size={26} /> : 'তথ্য আপডেট করুন'}
        </button>
      </div>
      ...
      <button onClick={() => supabase.auth.signOut()} className="w-full py-6 bg-red-500 text-white font-black rounded-[2.2rem] shadow-xl active:scale-[0.95] transition-all flex items-center justify-center gap-4 text-lg border-2 border-red-400"><LogOut size={26} /> লগ আউট</button>
    </div>
  );
};
