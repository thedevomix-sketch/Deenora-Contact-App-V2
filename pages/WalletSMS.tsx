...
import { CreditCard, Loader2, Send, ChevronDown, BookOpen, Users, CheckCircle2, MessageSquare, Plus, Edit3, Trash2, Smartphone } from 'lucide-react';
...
const WalletSMS: React.FC<WalletSMSProps> = ({ lang, madrasah, triggerRefresh, dataVersion }) => {
  ...
  const handleSendBulk = async () => {
    ...
    try {
      ...
      setBulkSuccess(true); setBulkMessage(''); setSelectedClassId('');
      triggerRefresh();
      ...
      {activeTab === 'bulk-sms' && (
        <div className="bg-white/95 backdrop-blur-xl p-8 rounded-[3rem] shadow-[0_30px_80px_rgba(46,11,94,0.3)] border border-white/50 space-y-8 animate-in slide-in-from-bottom-5">
          <div className="space-y-6">
            ...
            <button onClick={handleSendBulk} disabled={sendingBulk || !bulkMessage.trim()} className="w-full py-6 premium-btn text-white font-black rounded-[2.2rem] shadow-[0_20px_50px_rgba(141,48,244,0.4)] active:scale-[0.98] transition-all flex items-center justify-center gap-4 text-xl font-noto disabled:opacity-30">
      ...
      {activeTab === 'templates' && (
        <div className="space-y-5 animate-in slide-in-from-bottom-5">
          <div className="flex items-center justify-between px-4">
            ...
            <button className="premium-btn text-white p-3.5 rounded-[1.2rem] shadow-xl border border-white/20"><Plus size={24} strokeWidth={4} /></button>
          </div>
          {templates.map(tmp => (
            <div key={tmp.id} className="bg-white/95 backdrop-blur-md p-7 rounded-[2.8rem] border border-white shadow-xl space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                 <h3 className="font-black text-[#2E0B5E] text-[20px] font-noto">{tmp.title}</h3>
                 <div className="flex gap-2">
                   <button className="w-11 h-11 bg-[#F2EBFF] text-[#8D30F4] rounded-xl flex items-center justify-center border border-[#8D30F4]/10"><Edit3 size={20} /></button>
                   <button className="w-11 h-11 bg-red-50 text-red-400 rounded-xl flex items-center justify-center border border-red-100"><Trash2 size={20} /></button>
                 </div>
              </div>
      ...
      {activeTab === 'recharge' && (
        <div className="bg-white/95 backdrop-blur-xl p-8 rounded-[3rem] shadow-2xl border border-white space-y-8 text-center animate-in slide-in-from-bottom-5">
          ...
          <form className="space-y-6 text-left">
            <div className="space-y-2"><label className="text-[11px] font-black text-[#4B168A] uppercase tracking-widest px-2">টাকার পরিমাণ</label><input type="number" className="w-full px-6 py-5 bg-[#F2EBFF] border-2 border-[#8D30F4]/10 rounded-[1.5rem] text-[#2D3142] font-black text-xl outline-none focus:border-[#8D30F4]" placeholder="Amount (৳)" /></div>
            <div className="space-y-2"><label className="text-[11px] font-black text-[#4B168A] uppercase tracking-widest px-2">TrxID</label><input type="text" className="w-full px-6 py-5 bg-[#F2EBFF] border-2 border-[#8D30F4]/10 rounded-[1.5rem] text-[#2D3142] font-black text-xl outline-none focus:border-[#8D30F4] uppercase" placeholder="8X23M1..." /></div>
