...
import { Loader2, Search, ChevronRight, User as UserIcon, ShieldCheck, Database, Globe, CheckCircle, XCircle, CreditCard, Save, X, Settings, Smartphone, MessageSquare } from 'lucide-react';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ lang, currentView = 'list', dataVersion = 0 }) => {
  ...
  const approveTransaction = async (tr: Transaction) => {
    ...
    try {
      ...
      // 2. Send SMS Notification to Madrasah Phone
      if (tr.madrasahs?.phone) {
        ...
        await smsApi.sendDirect(tr.madrasahs.phone, msg);
      }

      setPendingTrans(p => p.filter(t => t.id !== tr.id));
      alert('Approved & Notification SMS Sent!');
      initData();
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xl z-[500] flex items-center justify-center p-8 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl space-y-6 relative overflow-y-auto max-h-[90vh]">
            ...
            <div className="text-center space-y-2">
               ...
               <p className="text-[10px] font-black text-[#8D30F4] uppercase tracking-widest">User Configuration</p>
            </div>

            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Caller ID (Sender Name)</label>
                <input type="text" className="w-full h-14 bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 font-black uppercase" value={editCallerId} onChange={(e) => setEditCallerId(e.target.value)} placeholder="e.g. MADRASAH" />
              </div>
              <div className="space-y-1.5">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Client ID</label>
                <input type="text" className="w-full h-14 bg-slate-50 border-2 border-slate-100 rounded-2xl px-5 font-black" value={editClientId} onChange={(e) => setEditClientId(e.target.value)} />
              </div>
              <div className="flex items-center justify-between bg-slate-50 p-4 rounded-2xl">
                <span className="text-sm font-black text-slate-700">Account Status</span>
            ...
            <button onClick={updateUserProfile} disabled={isUpdatingUser} className="w-full h-16 premium-btn text-white font-black rounded-full flex items-center justify-center gap-3 shadow-xl">
               {isUpdatingUser ? <Loader2 className="animate-spin" size={24} /> : <><Save size={24} /> Update Profile</>}
            </button>
          </div>
        </div>
      )}
      ...
      {view === 'gateway' && (
        <div className="bg-white p-8 rounded-[3rem] shadow-2xl space-y-8">
           <div className="flex items-center gap-4">
              ...
              <h3 className="text-xl font-black text-slate-800">Global System Settings</h3>
           </div>
          ...
          {pendingTrans.length > 0 ? pendingTrans.map(tr => (
            <div key={tr.id} className="bg-white p-7 rounded-[2.5rem] border border-slate-100 shadow-xl space-y-5">
              <div className="flex items-start justify-between">
                ...
                <Smartphone className="text-slate-200" size={32} />
              </div>
              ...
              <div className="flex gap-3">
                 <button onClick={() => approveTransaction(tr)} className="flex-1 py-4 bg-green-500 text-white font-black rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-green-100"><CheckCircle size={18} /> Approve & SMS</button>
